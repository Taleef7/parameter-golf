...
class Hyperparameters:
    ...
    random_map_adapter_enabled = bool(int(os.environ.get("RANDOM_MAP_ADAPTER_ENABLED", "0")))
    random_map_adapter_rank = int(os.environ.get("RANDOM_MAP_ADAPTER_RANK", "0"))
    random_map_adapter_layers = os.environ.get("RANDOM_MAP_ADAPTER_LAYERS", "")
...
SUPPORTED_RANDOM_MAP_TARGETS = ("q", "v")


@dataclass(frozen=True)
class RandomMapAdapterConfig:
    enabled: bool
    rank: int
...
def parse_random_map_adapter_layers(spec: str, num_layers: int) -> tuple[int, ...]:
    ...
    for raw in cleaned.split(","):
        token = raw.strip()
        if not token:
            raise ValueError("RANDOM_MAP_ADAPTER_LAYERS must be a comma-separated list of integers")
        try:
            layer_idx = int(token)
        except ValueError as exc:
            raise ValueError(f"RANDOM_MAP_ADAPTER_LAYERS contains non-integer layer {token!r}") from exc
        if layer_idx < 0 or layer_idx >= num_layers:
            raise ValueError(
                f"RANDOM_MAP_ADAPTER_LAYERS index {layer_idx} is out of range for NUM_LAYERS={num_layers}"
            )
        if layer_idx not in values:
            values.append(layer_idx)
    return tuple(values)


def parse_random_map_adapter_targets(spec: str) -> tuple[str, ...]:
    cleaned = spec.strip()
    if not cleaned:
        raise ValueError("RANDOM_MAP_ADAPTER_TARGETS must list at least one of q,v")
    values: list[str] = []
    for raw in cleaned.split(","):
        token = raw.strip().lower()
        if not token:
            raise ValueError("RANDOM_MAP_ADAPTER_TARGETS must be a comma-separated list without empty entries")
        if token not in SUPPORTED_RANDOM_MAP_TARGETS:
            raise ValueError(
                f"RANDOM_MAP_ADAPTER_TARGETS contains unsupported target {token!r}; expected one of {SUPPORTED_RANDOM_MAP_TARGETS}"
            )
...
def build_random_map_adapter_config(args: Hyperparameters) -> RandomMapAdapterConfig:
    if not args.random_map_adapter_enabled:
        return RandomMapAdapterConfig(
            enabled=False,
            rank=0,
            layer_indices=(),
            targets=(),
    ...
    if args.random_map_adapter_rank <= 0:
        raise ValueError(
            f"RANDOM_MAP_ADAPTER_RANK must be positive when enabled, got {args.random_map_adapter_rank}"
        )
    layer_indices = parse_random_map_adapter_layers(args.random_map_adapter_layers, args.num_layers)
    if not layer_indices:
        raise ValueError("RANDOM_MAP_ADAPTER_LAYERS must name at least one layer when enabled")
    targets = parse_random_map_adapter_targets(args.random_map_adapter_targets)
    return RandomMapAdapterConfig(
        enabled=True,
        rank=args.random_map_adapter_rank,
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.q_adapter is not None:
            q = q + self.q_adapter(x)
        q = q.reshape(bsz, seqlen, self.num_heads, self.head_dim)
        k = F.linear(x, k_w.to(x.dtype)).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
        v = F.linear(x, v_w.to(x.dtype))
        if v_embed is not None:
            v = v + v_embed
        if self.v_adapter is not None:
            v = v + self.v_adapter(x)
...
class RandomLinearMapAdapter(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, rank: int, seed: int, scale_init: float):
        ...
        random_map = torch.randn(rank, input_dim, generator=generator, dtype=torch.float32) / math.sqrt(input_dim)
        self.register_buffer("random_map", random_map, persistent=True)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(
            ...
            random_map_targets=random_map_targets,
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        random_map_adapter_config: RandomMapAdapterConfig | None = None,
    ):
        super().__init__()
        ...
        adapter_layer_set = set(self.random_map_adapter_layers)
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    gated_attention=gated_attention,
                    value_residual=value_residual,
                    random_map_targets=self.random_map_adapter_targets if i in adapter_layer_set else (),
        ...
        self.random_map_adapter_parameter_count = sum(
            p.numel()
            for name, p in self.blocks.named_parameters()
            if ".q_adapter." in name or ".v_adapter." in name
        )
        self.random_map_adapter_buffer_count = sum(
            b.numel()
            for name, b in self.blocks.named_buffers()
            if name.endswith(".random_map")
        )
        self._init_weights()
...
def main() -> None:
    ...
    random_map_adapter_config = build_random_map_adapter_config(args)
    base_model = GPT(
        ...
        random_map_adapter_config=random_map_adapter_config,
    ).to(device).bfloat16()
    ...
    scalar_params = [
        ...
        and ".q_adapter." not in name
        and ".v_adapter." not in name
    ]
    scalar_params.extend(adapter_params)
    ...
    log0(
        ...
        f"random_map_buffer_params:{base_model.random_map_adapter_buffer_count}"
    )
